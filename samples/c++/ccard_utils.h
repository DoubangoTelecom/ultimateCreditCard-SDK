...
* File description
*/
struct CreditCardFile {
	void* uncompressedData = nullptr;
	size_t width = 0;
	size_t height = 0;
	ULTCCARD_SDK_IMAGE_TYPE type;

	virtual ~CreditCardFile() {
		release();
	}
...
* @returns 
*/
static bool alprDecodeFile(const std::string& path, CreditCardFile& alprFile)
{
	ULTCCARD_SDK_ASSERT(!path.empty());
...
#	endif
	if (!file) {
		ULTCCARD_SDK_PRINT_ERROR("Failed to open file at: %s", path.c_str());
		return false;
	}
	...
	if (!uncompressedData || width <= 0 || height <= 0 || (channels != 1 && channels != 3 && channels != 4)) {
		ULTCCARD_SDK_PRINT_ERROR("Invalid file(%s, %d, %d, %d)", path.c_str(), width, height, channels);
		if (uncompressedData) {
			free(uncompressedData);
		}
		return false;
	...
	// List of supported types: https://www.doubango.org/SDKs/ccard/docs/cpp-api.html#_CPPv4N15ultimateCreditCardSdk22ULTCCARD_SDK_IMAGE_TYPEE
	alprFile.type = (channels == 3) ? ULTCCARD_SDK_IMAGE_TYPE_RGB24 : (channels == 1 ? ULTCCARD_SDK_IMAGE_TYPE_Y : ULTCCARD_SDK_IMAGE_TYPE_RGBA32);
