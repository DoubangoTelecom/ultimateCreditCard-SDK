 ...
 */
package org.doubango.ultimateCreditCard.Benchmark;
...
import org.doubango.ultimateCreditCard.Sdk.*;
...
public class CCardBenchmarkActivity extends AppCompatActivity {

    /**
     * TAG used for the debug logs.
     */
    static final String TAG = CCardBenchmarkActivity.class.toString();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        UltCreditCardSdkResult result = assertIsOk(UltCreditCardSdkEngine.init(
                getAssets(),
                getConfig()
        ));
    }
    ...
    protected void onResume() {
        ...
        // Read the images
        final CCardImage images[] = new CCardImage[2];
        images[0] = readFile("paymentsense.jpg");
        if (images[0] == null) {
            throw new AssertionError("Failed to read file");
        }
        images[1] = readFile("revolut.jpg");
        if (images[1] == null) {
            throw new AssertionError("Failed to read file");
        }

        // Warm up to prepare for benchmark
        assertIsOk(UltCreditCardSdkEngine.warmUp(images[1].mType));
        ...
        for (Integer i : indices) {
            final CCardImage image = images[i];
            assertIsOk(UltCreditCardSdkEngine.process(
                    image.mType,
                    image.mBuffer,
                    image.mWidth,
                    image.mHeight
    ...
    public void onDestroy() {
        Log.i(TAG, "onDestroy");
        final UltCreditCardSdkResult result = assertIsOk(UltCreditCardSdkEngine.deInit());

        super.onDestroy();
    }

    CCardImage readFile(final String name) {
        ...
        return new CCardImage(ULTCCARD_SDK_IMAGE_TYPE.ULTCCARD_SDK_IMAGE_TYPE_RGBA32, nativeBuffer, width, height);
    }
    ...
    static class CCardImage {
        final ULTCCARD_SDK_IMAGE_TYPE mType;
        final ByteBuffer mBuffer;
        final int mWidth;
        final int mHeight;

        CCardImage(final ULTCCARD_SDK_IMAGE_TYPE type, final ByteBuffer buffer, final int width, final int height) {
            mType = type;
            mBuffer = buffer;
            mWidth = width;
            mHeight = height;
     ...
     */
    static final UltCreditCardSdkResult assertIsOk(final UltCreditCardSdkResult result) {
        if (!result.isOK()) {
     ...
     */
    static final String resultToString(final UltCreditCardSdkResult result) {
